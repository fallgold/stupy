...
tpl_modifier_list:
		r_variable { $$ = $1; }
	|	r_variable ';' { $$ = $1; }
	|	T_CONSTANT_ENCAPSED_STRING '|' T_STRING { $2.u.op.opline_num = zend_do_begin_function_call(&$3, 1 TSRMLS_CC); zend_do_pass_param(&$1, ZEND_SEND_VAL TSRMLS_CC); }
			tpl_modifier_param_list { zend_do_end_function_call(&$3, &$$, 0, $2.u.op.opline_num TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
	|	tpl_modifier_list '|' T_STRING { $2.u.op.opline_num = zend_do_begin_function_call(&$3, 1 TSRMLS_CC); zend_do_pass_param(&$1, ZEND_SEND_VAL TSRMLS_CC); }
			tpl_modifier_param_list { zend_do_end_function_call(&$3, &$$, 0, $2.u.op.opline_num TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
;
...
tpl_modifier_param_list:
	...
	|	tpl_modifier_param_list ':' { STU_G(st_tpl_no_constant) = 1; } expr { STU_G(st_tpl_no_constant) = 0; Z_LVAL($$.u.constant)=Z_LVAL($1.u.constant)+1;  zend_do_pass_param(&$4, ZEND_SEND_VAL TSRMLS_CC); }
;
...
tpl_statement:
	...
	|	T_START_TPL_TRANSLATE tpl_encaps_list T_END_TPL { int is_dynamic = zend_do_begin_function_call(&$1, 1 TSRMLS_CC);
			zend_do_pass_param(&$2, ZEND_SEND_VAL TSRMLS_CC); Z_LVAL($3.u.constant) = 1;
			zend_do_end_function_call(&$1, &$$, 0, is_dynamic TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); zend_do_echo(&$$ TSRMLS_CC); }
	/* function & tag */
	...
	/* for */
	|	T_START_TPL T_FOR '(' 
			for_expr ';' { zend_do_free(&$4 TSRMLS_CC); $5.u.op.opline_num = get_next_op_number(CG(active_op_array)); }
...
foreach_variable:
	...
	/*comment to resolved reduce/reduce conflict*/
