...
void tpl_do_include(znode *result, znode *file TSRMLS_DC) /* {{{ */
{
	...
	if (!STU_G(tpl_include_extend)) { // do include
		...
		zend_do_pass_param(file, ZEND_SEND_VAL TSRMLS_CC);
		Z_LVAL(check_param.u.constant) = STU_G(tpl_compile_time);
		Z_TYPE(check_param.u.constant) = IS_LONG;
		check_param.op_type = IS_CONST;
		zend_do_pass_param(&check_param, ZEND_SEND_VAL TSRMLS_CC);
		Z_LVAL(check_param.u.constant) = 2;
		zend_do_end_function_call(&check_func, &check_result, 0, 0 TSRMLS_CC);
		zend_do_extended_fcall_end(TSRMLS_C);
...
void tpl_do_function(znode *result, znode *modifier, znode *attr_list, znode *open_token TSRMLS_DC) /* {{{ */
{
	...
	if (orig_mod != NULL) { 
		...
		zend_do_pass_param(&tag_param, ZEND_SEND_VAL TSRMLS_CC); 
	}
	zend_do_pass_param(attr_list, ZEND_SEND_VAL TSRMLS_CC);
	Z_LVAL(open_token->u.constant) = offset;
	zend_do_end_function_call(modifier, result, 0, is_dynamic TSRMLS_CC); 
	zend_do_extended_fcall_end(TSRMLS_C);
	if (orig_mod != NULL) { 
		if (!STU_G(tpl_tag_symbol)) {
			// extend tag_content
			...
			zend_do_pass_param(&tag_param, ZEND_SEND_VAL TSRMLS_CC); 
			zend_do_pass_param(result, ZEND_SEND_VAL TSRMLS_CC); 
			Z_LVAL(open_token->u.constant) = 2;
			zend_do_end_function_call(&post_tag_func, result, 0, 0 TSRMLS_CC);
			zend_do_extended_fcall_end(TSRMLS_C);
		}
		zend_do_free(result TSRMLS_CC);
	} else {
