...
int zend_do_begin_function_call(znode *function_name, zend_bool check_namespace TSRMLS_DC) /* {{{ */
{
	...
	if (zend_hash_find(CG(function_table), lcname, function_name->u.constant.value.str.len+1, (void **) &function)==FAILURE) {
		if (!STU_G(st_tpl_with_script)) {
			...
			sprintf(mod, "stupy_mod_%s", lcname); 
			if (zend_hash_find(CG(function_table), mod, mod_len+1, (void **) &function)==FAILURE) {
				// add auto load tpl modifier
				int dummy = 1;
				zend_hash_update(&STU_G(tpl_modifiers), lcname, function_name->u.constant.value.str.len + 1, (void *)&dummy, sizeof(int), NULL);
				if (STU_G(tpl_mod_pattern)) {
					// user defined modifier pattern
					efree(mod);
					mod_len = function_name->u.constant.value.str.len + strlen(STU_G(tpl_mod_pattern)) - 2; // minus %s
					mod = safe_emalloc(mod_len, 1, 1);
					sprintf(mod, STU_G(tpl_mod_pattern), lcname);
					zval_dtor(&function_name->u.constant);
					Z_STRVAL(function_name->u.constant) = mod;
					Z_STRLEN(function_name->u.constant) = mod_len;
				}
				zend_do_begin_dynamic_function_call(function_name, 0 TSRMLS_CC);
				efree(lcname);
				return 1; /* Dynamic */
			} else {
				// found, use default modifier prefix
				efree(lcname);
				lcname = mod;
				function_name->u.constant.value.str.len = mod_len;
			}
		} else {
			zend_do_begin_dynamic_function_call(function_name, 0 TSRMLS_CC);
			efree(lcname);
	...
	} else if ((CG(compiler_options) & ZEND_COMPILE_IGNORE_INTERNAL_FUNCTIONS) &&
...
void tpl_do_include(znode *result, znode *file TSRMLS_DC) /* {{{ */
{
	...
	if (!STU_G(tpl_include_extend)) { // do include
		zend_do_include_or_eval(ZEND_INCLUDE, result, file TSRMLS_CC);

	} else if (STU_G(tpl_include_extend) == 1 || STU_G(tpl_include_extend) == 2) { // if subtpl_mtime > maintpl_mtime then include else extend, and if 1==tpl_include_extend then trigger warning
		...
		zend_do_begin_function_call(&check_func, 0 TSRMLS_CC);
		zend_do_pass_param(file, ZEND_SEND_VAL, 1 TSRMLS_CC);
		Z_LVAL(check_param.u.constant) = STU_G(tpl_compile_time);
		Z_TYPE(check_param.u.constant) = IS_LONG;
		check_param.op_type = IS_CONST;
		zend_do_pass_param(&check_param, ZEND_SEND_VAL, 2 TSRMLS_CC);
		Z_LVAL(check_param.u.constant) = 2;
		...
		{
			if (file_ok) {
				Z_STRVAL(file->u.constant) = estrndup(Z_STRVAL(file->u.constant), Z_STRLEN(file->u.constant));
				zend_do_include_or_eval(ZEND_INCLUDE, result, file TSRMLS_CC);
			}
		}
		zend_do_if_end(TSRMLS_C);
...
// do function or tag
// tag take precedence than function
void tpl_do_function(znode *result, znode *modifier, znode *attr_list, znode *open_token TSRMLS_DC) /* {{{ */
{
	...
	if (zend_hash_find(&STU_G(tpl_tags), Z_STRVAL(modifier->u.constant), Z_STRLEN(modifier->u.constant)+1, (void **) &tag_content) == SUCCESS
		...
		if (STU_G(tpl_tag_symbol)) {
			...
			Z_STRVAL(modifier->u.constant) = estrdup("_stupy_pre_tag");
			Z_STRLEN(modifier->u.constant) = 14;
		}
	}
	...
	if (orig_mod != NULL) { 
		...
		Z_STRVAL(tag_param.u.constant) = orig_mod; 
		Z_STRLEN(tag_param.u.constant) = orig_mod_len; 
		zend_do_pass_param(&tag_param, ZEND_SEND_VAL, offset++ TSRMLS_CC); 
	}
	zend_do_pass_param(attr_list, ZEND_SEND_VAL, offset TSRMLS_CC);
	Z_LVAL(open_token->u.constant) = offset;
	zend_do_end_function_call(modifier, result, open_token, 0, is_dynamic TSRMLS_CC); 
	zend_do_extended_fcall_end(TSRMLS_C);
	if (orig_mod != NULL) { 
		if (!STU_G(tpl_tag_symbol)) {
			// extend tag_content
			...
			post_tag_func.op_type = IS_CONST;
			zend_do_begin_function_call(&post_tag_func, 0 TSRMLS_CC);
			Z_STRVAL(tag_param.u.constant) = estrndup(Z_STRVAL(tag_param.u.constant), Z_STRLEN(tag_param.u.constant)); // fix String is not zero-terminated
			zend_do_pass_param(&tag_param, ZEND_SEND_VAL, 1 TSRMLS_CC); 
			zend_do_pass_param(result, ZEND_SEND_VAL, 2 TSRMLS_CC); 
			Z_LVAL(open_token->u.constant) = 2;
			zend_do_end_function_call(&post_tag_func, result, open_token, 0, 0 TSRMLS_CC);
			zend_do_extended_fcall_end(TSRMLS_C);
		}
		zend_do_free(result TSRMLS_CC);
	} else {
		// function result
		zend_do_echo(result TSRMLS_CC);
	}
}
/* }}} */
