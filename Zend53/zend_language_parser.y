...
%token T_END_TPL		 "tpl right delim (T_END_TPL)"
%token T_ATTR_FILE		 "attr file (T_ATTR_FILE)"
%token T_IN				 "in (T_IN)"
%token T_START_TPL_TRANSLATE "{t (T_START_TPL_TRANSLATE)"
%token T_TPL_INCLUDE	 "{include (T_TPL_INCLUDE)"
...
top_statement:
	...
	|	tpl_statement					{ zend_verify_namespace(TSRMLS_C); }
;
...
unticked_statement:
	...
	|	T_WHILE '(' { OPLINE_NUM($1) = get_next_op_number(CG(active_op_array));  } expr  ')' { zend_do_while_cond(&$4, &$5 TSRMLS_CC); } while_statement { zend_do_while_end(&$1, &$5 TSRMLS_CC); }
	|	T_DO { OPLINE_NUM($1) = get_next_op_number(CG(active_op_array));  zend_do_do_while_begin(TSRMLS_C); } statement T_WHILE '(' { OPLINE_NUM($5) = get_next_op_number(CG(active_op_array)); } expr ')' ';' { zend_do_do_while_end(&$1, &$5, &$7 TSRMLS_CC); }
	|	T_FOR
			'('
				for_expr
			';' { zend_do_free(&$3 TSRMLS_CC); OPLINE_NUM($4) = get_next_op_number(CG(active_op_array)); }
				for_expr
	...
	|	T_FOREACH '(' expr_without_variable T_AS
		...
		foreach_statement { zend_do_foreach_end(&$1, &$4 TSRMLS_CC); }
	|	T_DECLARE { OPLINE_NUM($1) = get_next_op_number(CG(active_op_array)); zend_do_declare_begin(TSRMLS_C); } '(' declare_list ')' declare_statement { zend_do_declare_end(&$1 TSRMLS_CC); }
	|	';'		/* empty statement */
...
tpl_modifier_list:
		r_variable { $$ = $1; }
	|	r_variable ';' { $$ = $1; }
	|	T_CONSTANT_ENCAPSED_STRING '|' T_STRING { OPLINE_NUM($2) = zend_do_begin_function_call(&$3, 1 TSRMLS_CC); zend_do_pass_param(&$1, ZEND_SEND_VAL, 1 TSRMLS_CC); }
			tpl_modifier_param_list { zend_do_end_function_call(&$3, &$$, &$5, 0, OPLINE_NUM($2) TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
	|	tpl_modifier_list '|' T_STRING { OPLINE_NUM($2) = zend_do_begin_function_call(&$3, 1 TSRMLS_CC); zend_do_pass_param(&$1, ZEND_SEND_VAL, 1 TSRMLS_CC); }
			tpl_modifier_param_list { zend_do_end_function_call(&$3, &$$, &$5, 0, OPLINE_NUM($2) TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
;

/* set st_tpl_no_constant = 1, means we will make T_STRING as T_CONSTANT_ENCAPSED_STRING */
tpl_modifier_param_list:
		/* empty */				{ Z_LVAL($$.u.constant) = 1; } /* first user param offset from 2, 1 is modified var or precede modifier result */
	|	tpl_modifier_param_list ':' { STU_G(st_tpl_no_constant) = 1; } expr { STU_G(st_tpl_no_constant) = 0; Z_LVAL($$.u.constant)=Z_LVAL($1.u.constant)+1;  zend_do_pass_param(&$4, ZEND_SEND_VAL, Z_LVAL($$.u.constant) TSRMLS_CC); }
;

tpl_inner_tpl_attr_name:
		T_STRING { $$ = $1; }
...
tpl_inner_tpl_attr_list:
		tpl_inner_tpl_attr_name '=' { if (Z_STRVAL($1.u.constant)[0] == '_') { zend_error(E_COMPILE_ERROR, "attribute name cannot start with _"); } STU_G(st_tpl_no_constant) = 1; } expr { STU_G(st_tpl_no_constant) = 0; zend_do_init_array(&$$, &$4, &$1, 0 TSRMLS_CC); }
	|	tpl_inner_tpl_attr_list tpl_inner_tpl_attr_name '=' { STU_G(st_tpl_no_constant) = 1; } expr { STU_G(st_tpl_no_constant) = 0; zend_do_add_array_element(&$$, &$5, &$2, 0 TSRMLS_CC); }
	|	tpl_inner_tpl_attr_anony { zend_do_init_array(&$$, &$1, NULL, 0 TSRMLS_CC); }
...
tpl_statement:
		/** {$a};  echo $a;
			...
			{$a|modifier:$aa:bb} echo modifier($a, $aa, 'bb'); */
		T_START_TPL tpl_modifier_list T_END_TPL { zend_do_echo(&$2 TSRMLS_CC); } 
		/* assign or any expr, do echo */
	|	T_START_TPL expr_without_variable T_END_TPL { 
			...
			zend_do_echo(&$2 TSRMLS_CC); 
		}
	|	T_START_TPL expr_without_variable ';' T_END_TPL { zend_do_echo(&$2 TSRMLS_CC); }
	/*|	T_START_TPL expr_without_variable ';' T_END_TPL { $$ = $2; } [> {$a = 1 + 2;} assign $a <]*/
	/*|	T_START_TPL expr_without_variable T_END_TPL { $$ = $2; } [> {$a = 1 + 2} assign $a <]*/
	/*|	T_START_TPL_WITH_ECHO echo_expr_list T_END_TPL [> {=$a,$b} echo $a, $b; <]*/
	/*|	T_START_TPL_WITH_ECHO echo_expr_list ';' T_END_TPL [> {=$a,$b;} echo $a, $b; <]*/
	...
	|	T_START_TPL T_IF expr T_END_TPL { zend_do_if_cond(&$3, &$3 TSRMLS_CC); } 
		/* attention: if 中的tpl_block，如果是空，会因为action而引起shift/reduce conflict, 故把所有action都移到下一个T_START_TPL后调用*/
		/* 原来为: tpl_block { zend_do_if_after_statement(&$5, 1 TSRMLS_CC); } */
		tpl_block T_START_TPL { zend_do_if_after_statement(&$3, 1 TSRMLS_CC); } 
		tpl_elseif_list tpl_else_single 
		tpl_endif { zend_do_if_end(TSRMLS_C); }
	/* for */
	|	T_START_TPL T_FOR '(' 
			for_expr ';' { zend_do_free(&$4 TSRMLS_CC); OPLINE_NUM($5) = get_next_op_number(CG(active_op_array)); }
			for_expr ';' { zend_do_extended_info(TSRMLS_C); zend_do_for_cond(&$7, &$8 TSRMLS_CC); }
			for_expr ')' T_END_TPL { zend_do_free(&$10 TSRMLS_CC); zend_do_for_before_statement(&$5, &$8 TSRMLS_CC); }
			tpl_block 
		T_START_TPL '/' T_FOR T_END_TPL { zend_do_for_end(&$8 TSRMLS_CC); }
	/* for in */
	|	T_START_TPL T_FOR '(' foreach_variable foreach_optional_arg T_IN expr ')' T_END_TPL 
			{ znode dummy_var; tpl_do_assign(&dummy_var, &$7 TSRMLS_CC); zend_do_if_cond(&dummy_var, &$1 TSRMLS_CC); zend_do_foreach_begin(&$2, &$3, &dummy_var, &$6, 0 TSRMLS_CC); zend_do_foreach_cont(&$2, &$3, &$6, &$4, &$5 TSRMLS_CC); }
			tpl_block T_START_TPL { zend_do_foreach_end(&$2, &$6 TSRMLS_CC); zend_do_if_after_statement(&$1, 1 TSRMLS_CC); }
...
additional_catch:
	T_CATCH '(' fully_qualified_class_name { OPLINE_NUM($$) = get_next_op_number(CG(active_op_array)); } T_VARIABLE ')' { zend_do_begin_catch(&$1, &$3, &$5, NULL TSRMLS_CC); } '{' inner_statement_list '}' { zend_do_end_catch(&$1 TSRMLS_CC); }
;
...
tpl_elseif_list:
		/* empty */
	|	tpl_elseif_list T_ELSEIF expr T_END_TPL { zend_do_if_cond(&$3, &$3 TSRMLS_CC); } tpl_block T_START_TPL { zend_do_if_after_statement(&$3, 0 TSRMLS_CC); }
	|	tpl_elseif_list T_ELSE T_IF expr T_END_TPL { zend_do_if_cond(&$4, &$4 TSRMLS_CC); } tpl_block T_START_TPL { zend_do_if_after_statement(&$4, 0 TSRMLS_CC); }
;
...
tpl_else_single:
		/* empty */
	|	T_ELSE T_END_TPL tpl_block T_START_TPL
;
...
expr_without_variable:
	...
	/* comment to unresolved reduce/reduce conflict of modifier and tpl assign {$a | $a} */
...
function:
	T_FUNCTION { OPLINE_NUM($$) = CG(zend_lineno); }
;
...
function_call:
		namespace_name '(' { OPLINE_NUM($2) = zend_do_begin_function_call(&$1, 1 TSRMLS_CC); }
				function_call_parameter_list
				')' { zend_do_end_function_call(&$1, &$$, &$4, 0, OPLINE_NUM($2) TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
	|	T_NAMESPACE T_NS_SEPARATOR namespace_name '(' { $1.op_type = IS_CONST; ZVAL_EMPTY_STRING(&$1.u.constant);  zend_do_build_namespace_name(&$1, &$1, &$3 TSRMLS_CC); OPLINE_NUM($4) = zend_do_begin_function_call(&$1, 0 TSRMLS_CC); }
				function_call_parameter_list
				')' { zend_do_end_function_call(&$1, &$$, &$6, 0, OPLINE_NUM($4) TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
