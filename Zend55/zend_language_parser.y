...
tpl_modifier_list:
		r_variable { $$ = $1; }
	|	r_variable ';' { $$ = $1; }
	|	T_CONSTANT_ENCAPSED_STRING '|' T_STRING { $2.u.op.opline_num = zend_do_begin_function_call(&$3, 1 TSRMLS_CC); zend_do_pass_param(&$1, ZEND_SEND_VAL, 1 TSRMLS_CC); }
			tpl_modifier_param_list { zend_do_end_function_call(&$3, &$$, &$5, 0, $2.u.op.opline_num TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
	|	tpl_modifier_list '|' T_STRING { $2.u.op.opline_num = zend_do_begin_function_call(&$3, 1 TSRMLS_CC); zend_do_pass_param(&$1, ZEND_SEND_VAL, 1 TSRMLS_CC); }
			tpl_modifier_param_list { zend_do_end_function_call(&$3, &$$, &$5, 0, $2.u.op.opline_num TSRMLS_CC); zend_do_extended_fcall_end(TSRMLS_C); }
;
